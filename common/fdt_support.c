...
static int yyt_config(void)
{
	unsigned char str[512] = {0x00};
	char yyt_config[675] = {0x00};
	struct mmc *mmc;
	int i;

	mmc = find_mmc_device(0);
	mmc_init(mmc);
	blk_select_hwpart_devnum(IF_TYPE_MMC, 0, 2);
	blk_dread(mmc_get_blk_desc(mmc), 0, 1, str);

	for (i = 0; i < 337; i++) {
		sprintf(yyt_config + 2*i, "%02x", str[i + 175]);
	}

	env_set("yyt_config", yyt_config);
...
int fdt_chosen(void *fdt)
{
	...
	yyt_config(); 
	test_str = env_get("yyt_config");
    fdt_setprop(fdt, nodeoffset, "yyt_config", test_str,
				  strlen(test_str) + 1);
